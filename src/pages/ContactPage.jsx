export default function ContactPage({ t, form, setForm, send }) {
      ...
      >
        <div>
          ...
          <h1
            style={{
              margin: 0,
              font: "400 clamp(34px, 5.4vw, 74px)/0.96 Anton, sans-serif",
          ...
          >
            {[
              ...
              >
                <span>{label}</span>
                <span style={{ color: '#E4DAF5' }}>{value}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div style={{ border: '1px solid #2A1E3A' }}>
          ...
          <div style={{ padding: 28, display: 'grid', gap: 20 }}>
            ...
            <Field label={t.tBusiness}>
              <input value={form.brand} onChange={set('brand')} style={inputStyle} />
            </Field>
            <Field label={t.tServiceNeeded}>
              <select
                value={form.service}
