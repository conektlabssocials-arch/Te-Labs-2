...
class Tower3D extends HTMLElement {
  ...
  connectedCallback() {
    ...
    // pan-y keeps page scroll; horizontal drag still rotates via OrbitControls
    ...
    const key = new THREE.PointLight(0xffc98c, 84, 30, 2);
    key.position.set(0, 0.9, 0);
    scene.add(key);
    const fill = new THREE.PointLight(0xffd9a6, 56, 40, 2);
    fill.position.set(2.6, 4.2, 2.2);
    const rim = new THREE.PointLight(0xffe8c4, 38, 34, 2);
    rim.position.set(-2.4, 8.2, -2.0);
    scene.add(rim);
    scene.add(fill);
    ...
    PLACES.forEach((p) => {
      ...
      s.position.set(Math.cos(p.a) * p.r, 0.7, Math.sin(p.a) * p.r);
      labels.add(s);
    });
    scene.add(labels);
    ...
    for (let i = 0; i < CARS; i++) {
      ...
      carDat.push({ a: a, lane: lane, r: 4 + Math.random() * 44, v: (2.4 + Math.random() * 3.2) * (Math.random() < 0.5 ? -1 : 1) });
    }
    ...
    const cars = new THREE.Points(carGeo, new THREE.PointsMaterial({
      size: 0.34, map: radialTexture("rgba(255,240,214,1)", "rgba(255,190,120,0)"),
    ...
    const tick = () => {
      ...
      for (let i = 0; i < CARS; i++) {
        const c = carDat[i];
        c.r += c.v * dt;
        if (c.r > 50) c.r = 4; else if (c.r < 4) c.r = 50;
